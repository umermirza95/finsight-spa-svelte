<script lang="ts">
	...
	import { 
		Plus, Search, Filter, ChevronDown, ListOrdered, X, Check, Trash2, ArrowLeft
	} from 'lucide-svelte';
	import { apiFetch } from '$lib/api';
	import { userStore } from '$lib/stores/auth';

	let targetCurrency = $state<string>('USD');

	let { walletId = undefined }: { walletId?: string } = $props();
	...
	let shouldBeSticky = $derived(graphHeight > 0 && windowHeight > 0 && graphHeight < windowHeight - 120);
	...
	async function loadData() {
		...
		try {
			...
			if (walletId) {
				const wRes = await apiFetch(`/api/Wallet/${walletId}`, {
					headers: { 'Authorization': `Bearer ${token}` }
				});
				if (wRes.ok) {
					const wData = await wRes.json();
					if (wData.data && wData.data.fsCurrencyCode) {
						targetCurrency = wData.data.fsCurrencyCode;
					}
				}
			} else if ($userStore && $userStore.defaultCurrency) {
				targetCurrency = $userStore.defaultCurrency;
			}
...
<div class="space-y-6 pb-12">
	...
	<div class="flex items-center">
		<a href="/wallets" class="flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground transition-colors px-3 py-1.5 rounded-lg hover:bg-secondary/50">
			<ArrowLeft size={16} />
			Back to Wallets
		</a>
	</div>
	...
	<div class="flex flex-col lg:flex-row gap-6 items-start relative">
			...
			<div class="p-4 md:p-6 border-b border-border/50 flex items-center justify-between w-full bg-card/50">
				...
				<div class="flex items-center gap-2">
					...
					<div class="relative actions-dropdown-container">
						...
						{#if isActionsOpen}
							<div class="absolute right-0 top-full mt-2 w-48 bg-popover border border-border/50 rounded-xl shadow-lg z-20 p-2">
								<a href={walletId ? `/wallets/${walletId}/import` : `/transactions/import`} class="block w-full text-left px-3 py-2 text-sm rounded-lg hover:bg-secondary/50 transition-colors text-foreground font-medium">Import Transactions</a>
							</div>
						{/if}
					</div>
				</div>
		...
		>
			...
			<TransactionsPieChart transactions={sortedTransactions} {categories} {targetCurrency} />
		</div>
	</div>
</div>
